...
export const LoginForm = (props: LoginFormProps) => {
    ...
    return (
        <div className="flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
            <h1 className="text-3xl font-bold text-white-1 mb-2">{t('login.signIn')}</h1>
            ...
            <Form method="post" className="flex flex-col gap-6">
                ...
                <p className="text-center text-sm text-light-gray-70 mt-4">
                    {t('login.noAccount')} <button type="button" onClick={() => setView("register")} className="text-primary hover:text-primary/70 font-medium tracking-wide bg-transparent border-none p-0 cursor-pointer">{t('login.registerNow')}</button>
                </p>
            </Form>
        </div>
    );
